...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'misho-zazil-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			misho_zazil()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'misho-zazil-navigation', 'async', true );
		wp_script_add_data( 'misho-zazil-navigation', 'precache', true );
		wp_localize_script(
			'misho-zazil-navigation',
			'mishoZazilScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'misho-zazil' ),
				'collapse' => __( 'Collapse child menu', 'misho-zazil' ),
			]
		);
	}
