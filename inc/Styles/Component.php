...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `misho_zazil()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'misho-zazil-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = misho_zazil()->get_asset_version( $css_dir . $data['file'] );
		...
		$color_theme_widget = get_theme_mod( 'color_theme_widget', '' );
		$color_theme_primary_shadow = get_theme_mod( 'color_theme_primary_shadow', '' );
		$color_theme_widget_shadow = get_theme_mod( 'color_theme_widget_shadow', '' );
		$color_theme_body_shadow = get_theme_mod( 'color_theme_body_shadow', '' );

		if ( empty( $color_theme_primary ) ) {
			$color_theme_primary = "#fb5012";
		}

		if ( empty( $color_theme_secondary ) ) {
			$color_theme_secondary = "#2480f2";
		}

		if ( empty( $color_theme_widget ) ) {
			$color_theme_widget = "#b4e33d";
		}

		if ( empty( $color_theme_primary_shadow ) ) {
			$color_theme_primary_shadow = "#e74004";
		}

		if ( empty( $color_theme_widget_shadow ) ) {
			$color_theme_widget_shadow = "#eded3b";
		}

		if ( empty( $color_theme_body_shadow ) ) {
			$color_theme_body_shadow = "#7d18e2";
		}

		$custom_css = ":root {
			--color-theme-primary: {$color_theme_primary}!important;
			--color-theme-primary-shadow: {$color_theme_primary_shadow}!important;
			--color-theme-body-shadow: {$color_theme_body_shadow}!important;
			--color-theme-secondary: {$color_theme_secondary}!important;
			--color-theme-widget: {$color_theme_widget}!important;
			--color-theme-widget-shadow: {$color_theme_widget_shadow}!important;

			--color-link: {$color_theme_secondary}!important;
			--color-link-visited: {$color_theme_secondary}!important;
			--color-link-active: {$color_theme_secondary}!important;

			--color-quote-border: {$color_theme_secondary}!important;
			--color-quote-citation: {$color_theme_secondary}!important;
		}";

		wp_add_inline_style( 'misho-zazil-global', $custom_css );
	}
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'misho-zazil-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'misho-zazil' ), $handle ) ), 'Misho: Zazil 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'misho_zazil_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! misho_zazil()->is_amp();
		 ...
		 */
		return apply_filters( 'misho_zazil_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'misho-zazil-sidebar'    => [
				...
				'preload_callback' => function() {
					return misho_zazil()->is_primary_sidebar_active();
				},
			],
			'misho-zazil-widgets'    => [
		 ...
		 */
		$css_files = apply_filters( 'misho_zazil_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'misho_zazil_google_fonts', $google_fonts );
